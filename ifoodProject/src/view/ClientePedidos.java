...
public class ClientePedidos extends Composite {
    ...
    public ClientePedidos(Composite parent, MainPage mainPage) {
        ...
        int idCliente = 2;
        List<Pedido> pedidosFeitos = pedidoBanco.listarPedidosPorCliente(idCliente);
    ...
    private void criarLayout(MainPage mainPage) {
        ...
        if (pedidoAberto.size() > 0) {
            criarSecaoPedidos(compositePedidos, "Aberto", pedidoAberto);
        }

        if (pedidoEmPreparo.size() > 0) {
            criarSecaoPedidos(compositePedidos, "Em Preparo", pedidoEmPreparo);
        }

        if (pedidoACaminho.size() > 0) {
            criarSecaoPedidos(compositePedidos, "A Caminho", pedidoACaminho);
        }

        if (pedidoEntregue.size() > 0) {
            criarSecaoPedidos(compositePedidos, "Entregue", pedidoEntregue);
        }

        if (pedidoCancelado.size() > 0) {
            criarSecaoPedidos(compositePedidos, "Cancelado", pedidoCancelado);
        }
    ...
    private void criarSecaoPedidos(Composite parent, String statusLabel, List<Pedido> pedidos) {
        ...
        for (Pedido pedido : pedidos) {
            ...
            Restaurante restaurante = obterInformacoesRestaurante(pedido.getIdRestaurante());
            ...
            lblPrecoTotal.setBounds(20, 40, 380, 20);
            ...
            lblItensPedido.setBounds(20, 100, 380, 40);
            
            Button btnVerMais = new Button(compositeCardPedido, SWT.NONE);
            btnVerMais.setText("Ver mais...");
            btnVerMais.setFont(localResourceManager.create(FontDescriptor.createFrom("Segoe UI", 10, SWT.NORMAL)));
            btnVerMais.setBounds(320, 140, 80, 25);
            btnVerMais.addSelectionListener(new SelectionAdapter() {
                @Override
                public void widgetSelected(SelectionEvent e) {
                    // Ação para mostrar mais detalhes
                }
            });
        }
    }

    private Restaurante obterInformacoesRestaurante(int idRestaurante) {
    	RestauranteBanco restauranteBanco = new RestauranteBanco(dbConnection);
        return restauranteBanco.visualizarRestaurante(idRestaurante);
    }
